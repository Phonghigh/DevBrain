...
| NestJS request validation (`class-validator` + `class-transformer` + `ValidationPipe`) | understood | 2026-07-18 | Introduced by DB1-03 — the api's first real route, `POST /captures`. A shared interface (`packages/shared`'s `CreateCaptureDto`) can't carry validation decorators, so a *separate class* in `apps/api` `implements` it and carries `@IsString()`/`@IsNotEmpty()`/`@IsOptional()`. A single global `ValidationPipe({whitelist:true, transform:true})` wires this into every route at once. Proved with real `curl` requests against the compiled server, not just an in-memory e2e test. **DB1-04 confirmed the same pattern extends cleanly to `@Query()`** (`@IsIn([...])` on an optional `status` filter) — no new concept needed, just reapplied. See [learn-log](learn-log/DB1-03-captures-post-endpoint.md) §4, [learn-log](learn-log/DB1-04-list-captures-endpoint.md) §4. |
| e2e test hygiene (clean up rows a test creates) | understood | 2026-07-18 | DB1-03's `captures.e2e-spec.ts` tags every row it creates (`source: 'e2e-test'`) and deletes them in `afterAll` — verified this actually works with a throwaway probe counting leftovers, not just trusted. Also cleaned up a stray row a manual `curl` boot-test left behind. See [learn-log](learn-log/DB1-03-captures-post-endpoint.md) §4, §6. |
...
- Covered: DB1-03 — `CapturesModule`: `POST /captures`, DevBrain's first real feature (paste a raw dump → it's saved as `status='raw'`). Added `class-validator`/`class-transformer` and a global `ValidationPipe({whitelist:true, transform:true})`; the shared `CreateCaptureDto` (a plain interface) can't carry validation decorators, so a *separate class* in `apps/api` implements it and carries them — the two stay in sync via `implements`, not by being the same declaration.
- `CapturesService.create` reused `toCaptureDto` (written in DB1-02) — the groundwork paid off one task later exactly as planned.
...
- Small tooling snag, not a code bug: tried to inspect `dev.db` directly via `better-sqlite3` from a Node script, got `Cannot find module` — it's a *transitive* dependency (of `@prisma/adapter-better-sqlite3`), not a direct one, and pnpm's strict node_modules layout doesn't expose those to `apps/api`'s own scripts. Used the already-direct `PrismaClient` import instead (same pattern as DB1-01's probe).
- Covered: DB1-04 — `GET /captures?status=raw`, the read side of DB1-03. Smooth, low-difficulty task: reused DB1-03's exact controller/service/DTO/mapper shape, just applied to a `@Query()` instead of a `@Body()` (`@IsIn(['raw','distilled','archived'])` on an optional `status` filter — same `ValidationPipe`, no new NestJS concept needed).
- The one thing worth remembering: proved "newest first" ordering by seeding test rows with **explicit, spaced-out `createdAt` values** via Prisma directly, rather than trusting that 3 `POST` requests fired back-to-back would land in different milliseconds — a subtly flaky test otherwise.
- Booted the compiled server again and drove real `curl` requests (create two captures, list them, confirm newest-first; confirm `?status=bogus` → 400) before calling it done.
- Stuck on / revisit next time: nothing blocking. Next eligible: DB1-05 (Web API client) — the first web-side code in Phase 1, a typed fetch wrapper for `createCapture`/`listCaptures`.
- Covered: DB1-05 — the web app's first API-calling code. `createCapture`/`listCaptures` typed entirely off `@devbrain/shared` (no shapes redeclared in `apps/web`), base URL from `VITE_API_BASE_URL` with a local-dev fallback. Low-difficulty, no real bugs — the interesting bit was confirming `import.meta.env.VITE_API_BASE_URL` typechecks at all, since this is the web app's first custom env var. Read Vite's actual `importMeta.d.ts` rather than guessing: `ImportMetaEnv` allows arbitrary keys as `any` by default, no extra `.d.ts` file needed.
